...
if "_PYTHON_PROJECT_BASE" in os.environ:
    ...
    _PROJECT_BASE = os.path.normpath(os.path.abspath(
        os.environ.get("_PYTHON_PROJECT_BASE")))
def is_python_build():
...
def _generate_posix_vars():
    ...
    generate_arg = [arg.split('=', 1)[1] for arg in sys.argv
                    if arg.startswith('--generate-posix-vars=')
                   ] or ['build/lib.%s-%s' % (get_platform(), sys.version[:3])]

    pybuilddir = generate_arg[0]
...
def _main():
    ...
    if any(arg.startswith('--generate-posix-vars=')
           for arg in sys.argv):
